...
<body <?php body_class(); ?>>
	...
	<a class="skip-link screen-reader-text" href="#primary"><?php esc_html_e( 'Skip to content', 'graceful-mode' ); ?></a>	
	...
	<div id="site-container">
		...
		<?php
			if ( is_home() && !is_paged() ) {
				$graceful_post_ticker_enabled = graceful_mode_options('post_ticker_show');

			    // Show Post Ticker if enabled
			    if ( $graceful_post_ticker_enabled ) {
			        get_template_part('template-parts/sections/post', 'ticker');
			    }
			    ...
			    $graceful_featured_boxes_enabled = graceful_mode_options('featured_boxes_show');
