...
if ( ! class_exists( 'WPTT_WebFont_Loader' ) ) {
	...
	class WPTT_WebFont_Loader {
		...
		public function get_local_files_from_css() {
			...
			foreach ( $font_files as $aster_storefront_font_family => $files ) {

				// The folder path for this font-family.
				$folder_path = $this->get_fonts_folder() . '/' . $aster_storefront_font_family;
		...
		public function get_remote_files_from_css() {
			...
			foreach ( $font_faces as $font_face ) {
				...
				$aster_storefront_style = explode( '}', $font_face )[0];

				// Sanity check.
				if ( false === strpos( $aster_storefront_style, 'font-family' ) ) {
					continue;
				}

				// Get an array of our font-families.
				preg_match_all( '/font-family.*?\;/', $aster_storefront_style, $matched_font_families );

				// Get an array of our font-files.
				preg_match_all( '/url\(.*?\)/i', $aster_storefront_style, $matched_font_files );

				// Get the font-family name.
				$aster_storefront_font_family = 'unknown';
				if ( isset( $matched_font_families[0] ) && isset( $matched_font_families[0][0] ) ) {
					$aster_storefront_font_family = rtrim( ltrim( $matched_font_families[0][0], 'font-family:' ), ';' );
					$aster_storefront_font_family = trim( str_replace( array( "'", ';' ), '', $aster_storefront_font_family ) );
					$aster_storefront_font_family = sanitize_key( strtolower( str_replace( ' ', '-', $aster_storefront_font_family ) ) );
				}

				// Make sure the font-family is set in our array.
				if ( ! isset( $result[ $aster_storefront_font_family ] ) ) {
					$result[ $aster_storefront_font_family ] = array();
				}
				...
				foreach ( $matched_font_files as $match ) {
					...
					// Add the file URL.
					$aster_storefront_font_family_url = rtrim( ltrim( $match[0], 'url(' ), ')' );

					// Make sure to convert relative URLs to absolute.
					$aster_storefront_font_family_url = $this->get_absolute_path( $aster_storefront_font_family_url );

					$result[ $aster_storefront_font_family ][] = $aster_storefront_font_family_url;
				}

				// Make sure we have unique items.
				// We're using array_flip here instead of array_unique for improved performance.
				$result[ $aster_storefront_font_family ] = array_flip( array_flip( $result[ $aster_storefront_font_family ] ) );
			}
			return $result;
		}
