...
$style = aster_storefront_page_header_style();

if ( $style ) {
    $classes[] = $style . '-page-header';
}

$visibility = get_theme_mod( 'aster_storefront_page_header_visibility', 'all-devices' );
...
$background_image_enabled = get_theme_mod('aster_storefront_page_header_style', true);

// Add background image to the header if enabled
$background_image = get_theme_mod( 'aster_storefront_page_header_background_image', '' );
$background_height = get_theme_mod( 'aster_storefront_page_header_image_height', '200' );
$inline_style = '';
...
$heading = get_theme_mod( 'aster_storefront_page_header_heading_tag', 'h1' );
$heading = apply_filters( 'aster_storefront_page_header_heading', $heading );

?>

<?php do_action( 'aster_storefront_before_page_header' ); ?>

<header class="<?php echo esc_attr( $classes ); ?>" style="<?php echo esc_attr( $inline_style ); ?> background-color: <?php echo esc_attr($aster_storefront_PAGE_TITLE_background_color); ?>;">
