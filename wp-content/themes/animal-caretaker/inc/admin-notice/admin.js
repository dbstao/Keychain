( function( $ ){
    $( document ).ready( function(){
      $( '.animal-caretaker-btn-get-started' ).on( 'click', function( e ) {
          e.preventDefault();
          $( this ).html( 'Processing.. Please wait' ).addClass( 'updating-message' );
          $.post( animal_caretaker_ajax_object.ajax_url, { 'action' : 'install_act_plugin' }, function( response ){
        ...
        $.ajax( ajaxurl,
          {
            type: 'POST',
            data: {
              action: 'animal_caretaker_dismissed_notice_handler',
              type: type,
            }
          } );
      } );
