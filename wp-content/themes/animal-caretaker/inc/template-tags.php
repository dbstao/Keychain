...
function animal_caretaker_enqueue_google_fonts() {
	...
	wp_enqueue_style( 'google-fonts-yeseva-one', 'https://fonts.googleapis.com/css2?family=Yeseva+One&display=swap' );

}
add_action( 'wp_enqueue_scripts', 'animal_caretaker_enqueue_google_fonts' );
...
function animal_caretaker_logo_title_description() {
	...
	?>  
	    <?php  if ( get_theme_mod( 'animal_caretaker_site_title_setting', true ) ) : ?>
			<a href="<?php echo esc_url( home_url( '/' ) ); ?>">
				<h2 class="site-title"><?php echo esc_html(get_bloginfo('name')); ?></h2>
			</a>
		<?php endif; ?>
	<?php
	    if ( get_theme_mod( 'animal_caretaker_tagline_setting', false ) ) :
