...
if ( !class_exists( 'Emoza_Custom_CSS' ) ) :

	/**
	 * Emoza_Custom_CSS 
	 */
	Class Emoza_Custom_CSS {
		...
		public function __construct() {	
			...
			$this->dynamic_css_uri  = trailingslashit( set_url_scheme( $upload_dir['baseurl'] ) ) . 'emoza/';
			$this->dynamic_css_path = trailingslashit( set_url_scheme( $upload_dir['basedir'] ) ) . 'emoza/';
		...
		public function output_css( $custom_css = false ) {
			...
			$css .= "input[type=\"text\"],input[type=\"email\"],input[type=\"url\"],input[type=\"password\"],input[type=\"search\"],input[type=\"number\"],input[type=\"tel\"],input[type=\"range\"],input[type=\"date\"],input[type=\"month\"],input[type=\"week\"],input[type=\"time\"],input[type=\"datetime\"],input[type=\"datetime-local\"],input[type=\"color\"],textarea,select,.woocommerce .select2-container .select2-selection--single,.woocommerce-page .select2-container .select2-selection--single,.woocommerce-account fieldset,.woocommerce-account .woocommerce-form-login, .woocommerce-account .woocommerce-form-register,.woocommerce-cart .woocommerce-cart-form .actions .coupon input[type=\"text\"] { border-color:" . esc_attr( $color_forms_borders ) . ";}" . "\n";
			...
			$css .= ".emoza-offcanvas-menu .main-navigation ul li { text-align:" . esc_attr( $mobile_menu_alignment ) . ";}" . "\n";
			...
			if ( $mobile_menu_link_separator ) {
				$css .= ".emoza-offcanvas-menu .main-navigation ul li { padding-top:5px;border-bottom: " . intval( $mobile_header_separator_width ) . "px solid " . esc_attr( $link_separator_color ) . ";}" . "\n";
			}

			$mobile_menu_link_spacing = get_theme_mod( 'mobile_menu_link_spacing', 20 );
			$css .= ".emoza-offcanvas-menu .main-navigation a { padding:" . esc_attr( $mobile_menu_link_spacing )/2 . "px 0;}" . "\n";
			...
			$css .= $this->get_background_color_css( 'offcanvas_menu_background', '', '.emoza-offcanvas-menu' );
			$css .= $this->get_color_css( 'offcanvas_menu_color', '', '.emoza-offcanvas-menu,.emoza-offcanvas-menu a:not(.button)' );
			$css .= $this->get_fill_css( 'offcanvas_menu_color', '', '.emoza-offcanvas-menu svg, .emoza-offcanvas-menu .dropdown-symbol .ws-svg-icon svg' );

			$offcanvas_mode = get_theme_mod( 'header_offcanvas_mode', 'layout1' );
			if ( 'layout2' === $offcanvas_mode ) {
				$css .= ".emoza-offcanvas-menu {max-width:100%;}" . "\n";
			}
			...
			//Quick view
			$css .= $this->get_background_color_css( 'content_cards_background', '', '.emoza-quick-view-popup .emoza-quick-view-popup-content' );

			//Mini cart
			$css .= $this->get_color_css( 'color_body_text', '', '.mini_cart_item a:nth-child(2)' );

			//Cart
			$css .= $this->get_stroke_css( 'color_link_hover', '', '.cross-sell-carousel .cross-sells .cross-sells-carousel-wrapper .emoza-cross-sell-nav svg path' );
			$css .= $this->get_stroke_css( 'color_link_default', '', '.cross-sell-carousel .cross-sells .cross-sells-carousel-wrapper .emoza-cross-sell-nav:hover svg path' );
			...
			$css .= $this->get_background_color_css( 'content_cards_background', '', '.woocommerce-message, .woocommerce-info, .woocommerce-error, .woocommerce-noreviews, p.no-comments, .emoza-quick-view-popup form.cart .qty' );
			...
			$css .= $this->get_color_css( 'color_body_text', '', '.woocommerce-cart-form .quantity .emoza-quantity-plus, form.cart .quantity .emoza-quantity-plus, .woocommerce-cart-form .quantity .emoza-quantity-minus, form.cart .quantity .emoza-quantity-minus' );
			...
			//View order
			$css .= $this->get_background_color_css( 'content_cards_background', '', '.woocommerce-account .emoza-wc-account-view-order+.woocommerce-notices-wrapper+p' );

			//Tables
			$css .= $this->get_border_color_rgba_css( 'color_body_text', '#212121', '.shop_table th, .shop_table td, .shop_table tr', '0.1' );
			...
			//Filter the value
			$css = apply_filters( 'emoza_custom_css_output', $css );
		...
		public function print_styles() {
			...
			wp_add_inline_style( 'emoza-style-min', $css );
		}
		...
		public function init() {
			...
			if ( false === get_transient( 'emoza_custom_css' ) ) {
		...
		public function update_custom_css_file() {
			...
			if ( $wp_filesystem->put_contents( $this->dynamic_css_path . 'custom-styles.css', $css ) ) {
				$this->clean_cache();
				set_transient( 'emoza_custom_css', true, 0 );
				return true;
			}
		...
		public function delete_custom_css_file() {
			...
			delete_transient( 'emoza_custom_css' );
		}
		 ...
		 */
		public static function get_border_color_rgba_css( $setting, $default, $selector, $opacity ) {
			$mod = get_theme_mod( $setting, $default );

			return $selector . '{ border-color:' . esc_attr( Emoza_Custom_CSS::get_instance()->to_rgba( $mod, $opacity ) ) . ';}' . "\n";
		}
		 ...
		 * Get stroke CSS
		 */
		public static function get_stroke_css( $setting, $selector, $default = '' ) {
			$mod = get_theme_mod( $setting, $default );
		...
		//Font sizes
		public static function get_font_sizes_css( $setting, $selector, $defaults = array() ) {
			$devices 	= array( 
		...
		//Top bottom padding
		public static function get_top_bottom_padding_css( $setting, $selector, $defaults = array() ) {
			$devices 	= array( 
				'desktop' 	=> '@media (min-width: 992px)',
				'tablet'	=> '@media (min-width: 576px) and (max-width:  991px)',
				'mobile'	=> '@media (max-width: 575px)'
		...
		//Left right padding
		public static function get_left_right_padding_css( $setting, $selector, $defaults = array() ) {
			$devices 	= array( 
				'desktop' 	=> '@media (min-width: 992px)',
				'tablet'	=> '@media (min-width: 576px) and (max-width:  991px)',
				'mobile'	=> '@media (max-width: 575px)'
