...
function emoza_sanitize_single_meta_elements( $input ) {
    ...
    foreach ( $input as $sub_value ) {
        if ( in_array( $sub_value, array( 'emoza_posted_on', 'emoza_posted_by', 'emoza_post_categories', 'emoza_entry_comments' ), true ) ) {
            $sanitized[] = $sub_value;
        }
    }
    return $sanitized;
...
function emoza_sanitize_product_loop_components( $input ) {
    $input      = (array) $input;
    $sanitized  = array();
    $elements   = array( 'woocommerce_template_loop_product_title', 'woocommerce_template_loop_rating', 'woocommerce_template_loop_price', 'emoza_loop_product_category', 'emoza_loop_product_description' );
