(function ($) {
  ...
  $("#install-activate-button").on("click", function (e) {
    ...
    var activationData = {
      ...
      nonce: blogpoet_localize.nonce,
    };
    $.post(blogpoet_localize.ajax_url, activationData, function (response) {
      ...
      if (match) {
        ...
        try {
          ...
          if (responseObj.success === true) {
            window.location.href = blogpoet_localize.redirect_url;
          } else {
            console.log("Error!");
          }
        } catch (error) {
  ...
  $("#blogpoet-dismiss-notice").on("click", ".notice-dismiss", function () {
    $.ajax({
      url: ajaxurl,
      data: {
        action: "blogpoet_dashboard_dismissble_notice",
        nonce: blogpoet_localize.nonce,
      },
    });
  });
