 ...
 * @package BeShop
 */
$beshop_theme = wp_get_theme();
$beshop_theme_version = $beshop_theme->get('Version');
$beshop_theme_domain = $beshop_theme->get('TextDomain');
if (!defined('BESHOP_VERSION')) {
	// Replace the version number of the theme on each release.
	define('BESHOP_VERSION', $beshop_theme_version);
}
...
if (!function_exists('beshop_is_woocommerce_activated')) {
	function beshop_is_woocommerce_activated()
	{
 ...
 */
add_filter('excerpt_more', 'beshop_exerpt_empty_string', 999);
function beshop_exerpt_empty_string($more)
{
	$beshop_blog_readmore = get_theme_mod('beshop_blog_readmore', esc_html__('Read More', 'beshop'));
	$readmore_text = sprintf('<a href="%s" class="d-block breadmore">%s</a>
', get_the_permalink(get_the_ID()), $beshop_blog_readmore);
	if (is_admin()) {
		return $more;
	}
	return $readmore_text;
}
function beshop_excerpt_length($length)
{
	if (is_admin()) {
		return $length;
	}
...
add_filter('excerpt_length', 'beshop_excerpt_length', 999);
